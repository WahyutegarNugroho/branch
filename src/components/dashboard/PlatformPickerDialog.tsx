    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-w-[500px] max-h-[85vh] bg-[#222] border border-white/10 text-white rounded-2xl shadow-2xl p-6 font-sans flex flex-col">
        <DialogHeader className="mb-4 shrink-0">
          <DialogTitle className="text-xl font-semibold text-center text-white mb-2">
            Pilih Platform
          </DialogTitle>
        ...
        <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
          {filteredPlatforms.length > 0 ? (
              ...
              {filteredPlatforms.map((platform) => {
                ...
                return (
                  <button
                    ...
                    className="flex flex-col items-center justify-center p-4 gap-3 bg-zinc-900/50 hover:bg-zinc-800 rounded-xl border border-white/5 hover:border-white/20 transition-all group"
                  >
                    <div 
            ...
            <div className="flex flex-col items-center justify-center py-12 text-zinc-500">
              <p>Platform tidak ditemukan.</p>
              <p className="text-xs mt-1">Anda tetap bisa mengetik manual di luar.</p>
            </div>
          )}
        </div>
      </DialogContent>
